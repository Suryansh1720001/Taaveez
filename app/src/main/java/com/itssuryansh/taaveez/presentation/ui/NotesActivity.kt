        ...
        if (!(PoemDes.html.isNullOrEmpty())) {
            if (!(TextUtils.isEmpty(itemTopic.trim { it <= ' ' }))) {
                ...
                lifecycleScope.launch {
                    viewModel.createNotes(
                        NotesEntity(
                            Topic = itemTopic, Poem = htmlContentPoemDes, Date = date,
                            CreatedDate = date
                        )
                    )
                    Toast.makeText(
...
private fun openNotes(id: Int, NotesDao: NotesDao) {
    ...
    lifecycleScope.launch {
        viewModel.getNotesById(id).collect { notesEntity ->
            if (notesEntity != null) {
                Topic = notesEntity.Topic
                PoemDes = notesEntity.Poem
    ...
    binding.btnUpdatePoem.setOnClickListener {
        ...
        if (!(Poem.isEmpty())) {
            if (!(TextUtils.isEmpty(Topic.trim { it <= ' ' }))) {
                lifecycleScope.launch {
                    viewModel.updateNotes(NotesEntity(id, Topic, Poem, date, CreatedDate))
...
private fun deleteRecordAlertDialog(id: Int, employeeDao: NotesDao) {
    ...
    binding?.btnDeleteYes?.setOnClickListener {
        lifecycleScope.launch {
            viewModel.deleteNotes(NotesEntity(id))
            Toast.makeText(
                applicationContext,
